  <div class="">
      ...
      <div class="flex flex-wrap justify-center text-center">
          ...
          <div class="w-1/3 p-2 mb-4">
              <a @if(Route::has($service['route'])) href="{{ route($service['route']) ?? '' }}" @else href="#" @endif wire:navigate class="p-2 h-full flex flex-col items-center text-decoration-none hover:bg-gray-400/10 rounded-2xl border border-gray-400/50 transition-colors duration-200">
                  @if(!empty($service['icon_name']))
                  <flux:icon name="{{ $service['icon_name'] }}" class="text-[#6B747D]" />
                  @elseif(!empty($service['icon']))
                  {!! $service['icon'] !!}
                  @endif
                  <div class="mt-2 font-bold text-lg">{{ $service['label'] }}</div>
