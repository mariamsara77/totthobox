...
new class extends Component {
    ...
    public function mount()
    {
        ...
        // লগইন করা স্টুডেন্ট হলে সরাসরি তার ক্লাস সিলেক্ট হবে
        if ($user && ($user->is_student || $user->hasRole('student')) && $user->class_level_id) {
            $this->selectedClassId = $user->class_level_id;
        } else {
            // গেস্ট বা অন্য সবার জন্য প্রথম ক্লাসটি ডিফল্ট
    ...
    {
        // লগইন না করা থাকলে সাবজেক্ট লিস্ট খালি (Null State) দেখাবে
        if (!Auth::check() || !$this->selectedClassId) {
            return collect();
        }

        $cacheKey = "subjects_for_class_{$this->selectedClassId}";

        return Cache::store('redis')->remember($cacheKey, 1800, function () {
            return ClassLevel::where('id', $this->selectedClassId)
                ->first()
                    ?->subjects()
