...
<section class="flex flex-col h-screen overflow-hidden p-2">
                                    ...
                                    @if($message->hasMedia('attachments'))
                                        <div class="mb-2 grid gap-1 {{ $message->getMedia('attachments')->count() > 1 ? 'grid-cols-2' : 'grid-cols-1' }}">
                                            @foreach($message->getMedia('attachments') as $media)
                                                <div class="relative rounded-xl overflow-hidden bg-black/5 border border-black/5 dark:border-white/10 group/media transition-transform active:scale-95">
                                                    @if(str_contains($media->mime_type, 'image'))
    ...
    @error('messageText')
        <div class="fixed top-6 left-1/2 -translate-x-1/2 bg-zinc-200 p-2 dark:bg-zinc-700 rounded-xl z-50 shadow-lg shadow-zinc-900/20"
            x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 6500)" x-transition:enter="ease-out duration-300"
            x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
            x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 scale-100"
            x-transition:leave-end="opacity-0 scale-90">
            <div class="flex items-center justify-between gap-2">
                <flux:icon icon="check-circle" class="text-red-500" />
                <p class="text-red-500">
                    {{ $message }}
                </p>
                <flux:button @click="show = false" size="sm" variant="ghost">&times;</flux:button>
            </div>
        </div>
    @enderror
   ...
   <div x-data="{ 
        ...
        init() {
            // URL change হলে loading এবং scale reset করার জন্য watch
            this.$watch('url', () => {
                this.loading = true;
                this.scale = 1;
                
                // ভিডিওর ক্ষেত্রে সোর্স আপডেট করার জন্য ছোট হ্যাক
                if (this.type === 'video') {
                    this.$nextTick(() => {
    ...
    @open-lightbox.window="
        ...
        url = $event.detail.original || $event.detail.url; 
    ...
    <div class="absolute top-0 inset-x-0 p-6 flex justify-between items-center z-[110] bg-gradient-to-b from-black/60 to-transparent text-white">
        <div class="flex flex-col">
            <span class="text-sm font-semibold tracking-wide" x-text="type.toUpperCase()"></span>
            <span class="text-[11px] opacity-70" x-text="caption"></span>
        </div>
        <div class="flex items-center gap-3">
            <template x-if="type === 'image'">
                <div class="flex items-center bg-white/10 rounded-lg p-1 mr-4">
                    <button @click="scale = Math.max(0.5, scale - 0.5)" class="p-2 hover:bg-white/10 rounded-md">
                        <flux:icon name="minus" class="size-4" />
                    </button>
                    <span class="px-3 text-xs font-mono" x-text="Math.round(scale * 100) + '%'"></span>
                    <button @click="scale = Math.min(3, scale + 0.5)" class="p-2 hover:bg-white/10 rounded-md">
                        <flux:icon name="plus" class="size-4" />
                    </button>
                </div>
            </template>

            <a :href="url" download class="p-2.5 rounded-full bg-white/10 hover:bg-white/20 transition-all shadow-lg">
                <flux:icon name="arrow-down-tray" class="size-5" />
            </a>
            
            <button @click="open = false" class="p-2.5 rounded-full bg-red-500/20 text-red-400 hover:bg-red-500/40 transition-all">
                <flux:icon name="x-mark" class="size-5" />
            </button>
        </div>
    </div>

    <div class="relative w-full h-full flex items-center justify-center overflow-hidden" @click.self="open = false">
        
        <div x-show="loading" class="absolute inset-0 flex items-center justify-center z-50 text-white">
            <flux:icon.loading class="size-10 shadow-2xl" />
        </div>

        <template x-if="type === 'image'">
            <div class="relative transition-transform duration-200 ease-out cursor-move"
                :style="`transform: scale(${scale})`" 
                @dblclick="scale === 1 ? scale = 2 : scale = 1">
                
                <img :src="thumb" x-show="loading" class="max-w-[90vw] max-h-[85vh] object-contain blur-md">
                
                <img :src="url" 
                    @load="loading = false" 
                    x-show="url"
                    class="max-w-[95vw] max-h-[90vh] object-contain rounded shadow-2xl transition-all"
